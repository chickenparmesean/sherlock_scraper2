...
exports.SherlockScraper = void 0;
...
class SherlockScraper {
     ...
     */
    async scrapeProfileImage(sherlockUrl, options) {
        const opts = { ...this.options, ...options };
        if (!(0, config_1.isValidSherlockUrl)(sherlockUrl)) {
            return {
                success: false,
        ...
        try {
            ...
            if (opts.convertToBase64) {
                const conversionResult = await image_utils_1.ImageUtils.convertToBase64(imageResult.profileImageUrl, opts.timeout || 10000);
            ...
            if (opts.includeMetadata) {
                result.metadata = {
                    ...
                    ...(result.profileImageBase64 && {
                        ...
                        imageSize: image_utils_1.ImageUtils.estimateDataUrlSize(result.profileImageBase64),
                    }),
                };
            }
            return result;
     ...
     * @private
     */
    async extractImageFromHtml(html, opts) {
        try {
            ...
            for (const selector of this.config.selectors.imageSelectors) {
                ...
                if (imgElement.length > 0) {
                    ...
                    if (src) {
                        const processedUrl = image_utils_1.ImageUtils.processImageUrl(src, this.config.selectors.baseUrl);
                        if (processedUrl && image_utils_1.ImageUtils.isImageUrl(processedUrl)) {
                            if (opts.enableLogging) {
     ...
     */
    async scrapeProfile(sherlockUrl, options) {
        const opts = { ...this.options, ...options };
                    ...
                    if (/^\d+$/.test(text)) {
                        ...
                        if (containerClass.includes('flex-col') && containerClass.includes('items-center')) {
                            ...
                            if (context.includes(`${number}Total`) && achievements.mediumsFound === 0 && number !== achievements.highsFound) {
                                ...
                                if (opts.enableLogging)
                                    console.log(`    ✅ Assigned ${number} to mediumsFound`);
                            }
                                ...
                                if (opts.enableLogging)
                                    console.log(`    ✅ Assigned ${number} to soloMediums`);
                            }
                        }
                    }
                });
...
exports.SherlockScraper = SherlockScraper;
//# sourceMappingURL=sherlock-scraper.js.map