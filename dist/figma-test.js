...
exports.testFigmaIntegration = testFigmaIntegration;
exports.generateSlideExample = generateSlideExample;
const figma_client_1 = __importDefault(require("./figma-client"));
/**
...
async function testFigmaIntegration() {
    try {
        ...
        const figma = new figma_client_1.default();
        ...
        console.log('\n📋 Creating slide copy...');
        const newFileKey = await figma.duplicateFigmaFile(TEMPLATE_FILE_KEY, `Test Slide - ${new Date().toISOString().split('T')[0]}`);
        ...
        const imageLayers = figma.findLayers(templateFile.document, '').filter(node => node.type === 'RECTANGLE' || node.type === 'ELLIPSE' || node.type === 'FRAME');
        console.log(`Found ${imageLayers.length} potential image containers:`);
        imageLayers.slice(0, 10).forEach(layer => {
...
async function generateSlideExample() {
    try {
        ...
        const newSlideKey = await figma.duplicateFigmaFile(TEMPLATE_FILE_KEY, 'Auditor Slide - John Doe');
        ...
        await figma.replaceImage(newSlideKey, 'PROFILE_IMAGE_NODE_ID', 'https://example.com/profile-image.png');
...
//# sourceMappingURL=figma-test.js.map